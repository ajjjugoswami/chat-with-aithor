...
export async function sendToChatGPT(messages: ChatMessage[], modelId: string): Promise<AIResponse> {
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: modelId === 'gpt-4' ? 'gpt-4' : 'gpt-3.5-turbo',
        messages: messages,
        max_tokens: 1000,
        temperature: 0.7,
      }),
...
export async function sendToGemini(messages: ChatMessage[], modelId: string): Promise<AIResponse> {
  const apiKey = getAPIKeyForModel(modelId);
...
export async function sendToClaude(messages: ChatMessage[], modelId: string): Promise<AIResponse> {
  const apiKey = getAPIKeyForModel(modelId);
...
export async function sendToAI(messages: ChatMessage[], modelId: string): Promise<AIResponse> {
